...
export function runDevChecks() {
  ...
  console.assert(
    m.line1.includes("META"),
    "MRZ line1 must include META for META variant",
  );

  // Test 4: ADN is uppercased in MRZ line2
  console.assert(
    m.line2.includes("XYZ123"),
    "MRZ line2 must contain uppercased ADN",
  );
  ...
  try {
    const el = React.createElement(HoloPassport as any, {
      title: "Test",
      subtitle: "Sub",
      seed: "S",
      variant: "REAL",
    ...
    console.assert(
      !!el,
      "HoloPassport element should be creatable without photoUrl",
    );
  } catch (e) {
