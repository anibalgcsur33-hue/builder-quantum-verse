...
export default function MetaverseMap() {
  ...
  const islands = [
    ...
    { name: 'Lanzarote', color: 'from-amber-400 to-orange-500', size: 'medium' },
    { name: 'Fuerteventura', color: 'from-yellow-400 to-amber-500', size: 'medium' },
    { name: 'La Palma', color: 'from-green-400 to-emerald-500', size: 'small' },
    { name: 'La Gomera', color: 'from-purple-400 to-violet-500', size: 'small' },
  ...
  const getPropertyColor = (status: string, featured: boolean) => {
    ...
    switch (status) {
      case 'available': return 'text-neon-emerald';
  ...
  const filteredProperties = properties.filter(property => {
    if (filters.zone && !property.location.toLowerCase().includes(filters.zone.toLowerCase())) {
      return false;
    }
    ...
    <div className={`${isFullscreen ? 'fixed inset-0 z-50' : 'min-h-screen pt-20'} bg-blue-dark text-white/90`}>
      <div className="container mx-auto container-padding h-full">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gradient mb-2">Mapa 3D Metaverso</h1>
            <p className="text-white/70">
              Explora propiedades reales en las Islas Canarias con nuestro mapa interactivo futurista
            </p>
          </div>
        ...
        {showFilters && (
          <div className="glass-card p-6 rounded-xl mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div>
                <label className="block text-white/70 mb-2 text-sm">Zona</label>
                <select
                  value={filters.zone}
        ...
        <div className="relative h-[700px] glass-card rounded-2xl overflow-hidden">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className={`p-2 rounded transition-colors ${
                    mapView === '3d' ? 'bg-neon-teal text-blue-dark' : 'hover:bg-white/10'
                  }`}
                  title="Vista 3D"
                >
                  <Layers size={16} />
                ...
                <button
                  ...
                  className={`p-2 rounded transition-colors ${
                    mapView === '2d' ? 'bg-neon-teal text-blue-dark' : 'hover:bg-white/10'
                  }`}
                  title="Vista 2D"
                >
                  <Map size={16} />
          ...
          <div className="absolute top-4 right-4 z-20 glass-card p-4 rounded-lg">
            <div className="text-center">
              <div className="text-2xl font-bold text-neon-teal">{filteredProperties.length}</div>
          ...
          >
            ...
            {islands.map((island, index) => (
              <div
                ...
                className={`absolute rounded-full bg-gradient-to-br ${island.color} opacity-30 ${
                  island.size === 'large' ? 'w-32 h-32' : 
                  island.size === 'medium' ? 'w-24 h-24' : 'w-16 h-16'
                }`}
                style={{
                  left: `${20 + index * 120}px`,
                  top: `${100 + (index % 2) * 80}px`,
                  transform: mapView === '3d' ? `translateZ(${index * 10}px)` : 'none'
                }}
              >
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-white font-bold text-xs">{island.name}</span>
                </div>
              </div>
            ))}
            ...
            {filteredProperties.map((property) => {
              ...
              const color = getPropertyColor(property.status, property.featured);
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute inset-0 w-8 h-8 rounded-full animate-ping ${
                    property.featured ? 'bg-neon-teal/30' : 'bg-neon-emerald/20'
                  }`}></div>
                  
                  {/* Main pin */}
                  <div className={`relative w-8 h-8 rounded-full glass-card backdrop-blur-sm border-2 flex items-center justify-center group-hover:scale-125 transition-all duration-300 ${
                    property.featured ? 'border-neon-teal shadow-neon-teal' : 'border-neon-emerald/50'
                  }`}>
                  ...
                  <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="glass-card px-3 py-1 rounded-lg whitespace-nowrap">
                      <div className="text-white font-medium text-sm">{property.title}</div>
                      <div className="text-neon-teal text-xs">€{property.price.toLocaleString()}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="glass-card max-w-2xl w-full rounded-2xl overflow-hidden">
              ...
              <div className="relative">
                ...
                <div className="absolute bottom-4 right-4">
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                    selectedProperty.status === 'available' ? 'bg-neon-emerald/20 text-neon-emerald' :
                    selectedProperty.status === 'reserved' ? 'bg-yellow-400/20 text-yellow-400' :
                    'bg-red-400/20 text-red-400'
                  }`}>
                    {selectedProperty.status === 'available' ? 'Disponible' :
                     selectedProperty.status === 'reserved' ? 'Reservado' : 'Vendido'}
                  </span>
                </div>
              </div>
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  ...
                  <div className="text-center">
                    <Bath className="text-neon-emerald mx-auto mb-1" size={20} />
                    <div className="font-bold">{selectedProperty.bathrooms}</div>
                    <div className="text-white/60 text-sm">Baños</div>
                  </div>
                  <div className="text-center">
                    <Square className="text-neon-teal mx-auto mb-1" size={20} />
      ...
      <style jsx>{`
        .grid-pattern {
          background-image: linear-gradient(rgba(14, 231, 231, 0.1) 1px, transparent 1px),
                            linear-gradient(90deg, rgba(14, 231, 231, 0.1) 1px, transparent 1px);
