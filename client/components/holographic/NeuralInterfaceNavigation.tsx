...
import { Brain, Zap, Eye, Home, Crown, Star, Sparkles, Target, Cpu, Waves } from "lucide-react";
...
interface LuxuryMansion {
  id: string;
  name: string;
  architectural_style: 'modern_glass' | 'classical_marble' | 'futuristic_crystal' | 'organic_flow' | 'quantum_phase';
...
interface BrainActivity {
  ...
  consciousness_state: 'focused' | 'relaxed' | 'creative' | 'analytical' | 'intuitive';
}
...
export default function NeuralInterfaceNavigation() {
  ...
  const [selectedMansion, setSelectedMansion] = useState<LuxuryMansion | null>(null);
  ...
  const [thoughtMode, setThoughtMode] = useState<'exploration' | 'connection' | 'immersion'>('exploration');
  const [mansions, setMansions] = useState<LuxuryMansion[]>(luxuryMansions);
  const sceneRef = useRef<THREE.Scene | null>(null);
  const brainMesh = useRef<THREE.Group | null>(null);
  const mansionMeshes = useRef<Array<{ mesh: THREE.Group, mansion: LuxuryMansion }>>([]);
  ...
  useEffect(() => {
    ...
    const createBrainInterface = () => {
      ...
      const brainGeometry = new THREE.SphereGeometry(8, 32, 16, 0, Math.PI * 2, 0, Math.PI);
      const brainMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          consciousness_state: { value: getConsciousnessValue(brainActivity.consciousness_state) },
      ...
      const neuralInterface = new THREE.Mesh(interfaceGeometry, interfaceMaterial);
    ...
    mansions.forEach((mansion) => {
      ...
      const mansionMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          activation_state: { value: getActivationValue(mansion.activation_state) },
    ...
    neuralConnections.forEach((connection) => {
      const targetMansion = mansions.find(m => m.id === connection.target_mansion);
      ...
      const midPoint = new THREE.Vector3().lerpVectors(brainPos, mansionPos, 0.5);
      midPoint.y += 8; // Arco elevado
      
      const curve = new THREE.QuadraticBezierCurve3(brainPos, midPoint, mansionPos);
    ...
    consciousnessGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    consciousnessGeometry.setAttribute('velocity', new THREE.BufferAttribute(velocities, 3));
    ...
    const consciousness = new THREE.Points(consciousnessGeometry, consciousnessMaterial);
    ...
    const onMouseClick = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        const clickedMansion = intersects[0].object.parent?.userData?.mansion as LuxuryMansion;
        if (clickedMansion) {
          ...
          setMansions(prevMansions => 
            prevMansions.map(m => ({
              ...m,
              activation_state: m.id === clickedMansion.id ? 'selected' : 'dormant'
            }))
          );
    ...
    function getConsciousnessValue(state: string): number {
      switch (state) {
        case 'focused': return 4.0;
        case 'relaxed': return 2.0;
        case 'creative': return 3.5;
        case 'analytical': return 4.5;
        case 'intuitive': return 3.0;
        default: return 2.5;
      }
    }
    ...
    const animate = () => {
      ...
      if (brainMesh.current) {
        brainMesh.current.children.forEach((child) => {
          if (child.material instanceof THREE.ShaderMaterial) {
            ...
            child.material.uniforms.brain_activity.value = brainActivity.activity_level;
            child.material.uniforms.neural_navigation.value = mentalNavigation ? 1.0 : 0.0;
          }
        });
      ...
      mansionMeshes.current.forEach(({ mesh, mansion }) => {
        mesh.children.forEach((child) => {
          if (child.material instanceof THREE.ShaderMaterial) {
            child.material.uniforms.time.value = time;
            child.material.uniforms.activation_state.value = getActivationValue(mansion.activation_state);
        ...
        // Flotación mental
        mesh.position.y = mansion.location.y + Math.sin(time + mansion.neural_resonance) * 0.5;
      ...
      if (consciousnessMaterial.uniforms) {
        ...
        consciousnessMaterial.uniforms.consciousness_level.value = brainActivity.activity_level;
      }
  ...
  const getStyleIcon = (style: string) => {
    switch (style) {
      case 'futuristic_crystal': return <Sparkles className="w-5 h-5 text-purple-400" />;
      case 'classical_marble': return <Crown className="w-5 h-5 text-amber-400" />;
      case 'modern_glass': return <Eye className="w-5 h-5 text-blue-400" />;
      case 'organic_flow': return <Waves className="w-5 h-5 text-green-400" />;
      case 'quantum_phase': return <Zap className="w-5 h-5 text-pink-400" />;
  ...
  return (
    <section className="relative w-full h-screen bg-black overflow-hidden">
      ...
      <div className="absolute top-6 left-6 right-6 z-20 flex justify-between items-start">
        <div className="neural-title">
          ...
          <p className="text-white/80 text-lg">
            Streams Neuronales • Mansiones Holográficas • Navegación Mental • Elegancia Infinita
          </p>
        </div>
        ...
        <div className="neural-controls">
          ...
          <button
            onClick={() => setThoughtMode(thoughtMode === 'exploration' ? 'immersion' : 'exploration')}
            className="thought-btn"
          >
            <Target className="w-6 h-6" />
            <span>Modo {thoughtMode === 'exploration' ? 'Exploración' : 'Inmersión'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-8 z-20 user-neural-panel">
        <div className="neural-header">
          ...
          <div className="user-info">
            ...
            <p className="text-white/70 text-sm mb-2">{currentUser.neural_implant_version}</p>
            
            <div className="neural-signature">
              <span className="text-neon-teal font-mono text-lg">{currentUser.neural_signature}</span>
            </div>
          </div>
        </div>
        ...
        <div className="brain-activity">
          ...
          <div className="activity-stats">
            ...
            <div className="activity-item">
              ...
              <span className="activity-value">{brainActivity.dominant_frequency} Hz</span>
            </div>
            <div className="activity-item">
              <span className="activity-label">Patrón:</span>
              <span className="activity-value">{brainActivity.neural_pattern}</span>
            </div>
            <div className="activity-item">
              <span className="activity-label">Estado:</span>
              <span className="activity-value">{brainActivity.consciousness_state}</span>
            </div>
          </div>
        ...
        <div className="user-stats">
          <div className="stat-grid">
            ...
            <div className="stat-item">
              ...
              <span className="stat-value">{currentUser.navigation_skill}%</span>
        ...
        <div className="absolute bottom-8 right-8 z-20 mansion-neural-panel">
          <div className="mansion-header">
            ...
            <div className="mansion-info">
              <h3 className="text-xl font-bold text-white mb-1">{selectedMansion.name}</h3>
          ...
          <div className="neural-specs">
            <div className="spec-grid">
              <div className="spec-item">
                ...
                <span className="spec-value">{Math.round(selectedMansion.mental_accessibility * 100)}%</span>
              </div>
              <div className="spec-item">
                <Eye className="w-4 h-4 text-blue-400" />
                <span className="spec-label">Detalle Holográfico:</span>
                <span className="spec-value">{selectedMansion.holographic_detail}%</span>
              </div>
              <div className="spec-item">
                <Waves className="w-4 h-4 text-green-400" />
                <span className="spec-label">Resonancia Neural:</span>
                <span className="spec-value">{selectedMansion.neural_resonance}/10</span>
              </div>
            </div>
          </div>
          ...
          {neuralConnections.map((connection) => (
            <div key={connection.id} className="connection-item">
              ...
              <div className="connection-details">
                <div className="connection-source">{connection.source_neuron}</div>
      ...
      <style jsx>{`
        ...
        .neural-btn:hover, .thought-btn:hover {
          transform: translateY(-3px);
          box-shadow: 0 15px 30px rgba(14, 231, 231, 0.4);
        }

        .user-neural-panel, .mansion-neural-panel {
        ...
        .neural-header, .mansion-header {
          display: flex;
          gap: 1.5rem;
          margin-bottom: 1.5rem;
        }

        .neural-avatar, .mansion-preview {
          width: 80px;
          height: 80px;
        ...
        .connection-strength, .connection-speed {
          font-size: 0.75rem;
          color: rgba(255, 255, 255, 0.7);
        }
        ...
        @keyframes neural-activity {
          0%, 100% { box-shadow: 0 0 10px rgba(14, 231, 231, 0.5); }
