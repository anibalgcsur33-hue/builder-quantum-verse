...
export function attachCityPins(
  ...
  CITIES.forEach((c) => {
    ...
    const ent = ds.entities.add({
      ...
      billboard: {
        image: pinBuilder
          .fromText("🏠", Cesium.Color.fromCssColorString(pinColor), 48)
          .toDataURL(),
        scale: 0.9,
...
export function flyThroughRegion(
  viewer: Cesium.Viewer,
  region: "peninsula" | "canarias" | "baleares",
) {
  const regionCities = CITIES.filter((c) => c.region === region);
  console.log(
    `🎬 Tour panorámico por ${region}: ${regionCities.length} ciudades`,
  );
...
export function showRegionClusters(viewer: Cesium.Viewer) {
  const regions = {
    peninsula: {
      lat: 40.4,
      lon: -3.7,
      count: CITIES.filter((c) => c.region === "peninsula").reduce(
    ...
    canarias: {
      lat: 28.3,
      lon: -15.5,
      count: CITIES.filter((c) => c.region === "canarias").reduce(
        (sum, c) => sum + (c.properties || 0),
        0,
      ),
    },
    baleares: {
      lat: 39.6,
      lon: 2.9,
      count: CITIES.filter((c) => c.region === "baleares").reduce(
        (sum, c) => sum + (c.properties || 0),
        0,
      ),
    },
