...
export default function AIConcierge() {
  ...
  const propertyDatabase: PropertyMatch[] = [
    {
      ...
      roi: 8.2,
      matchScore: 95,
      reasons: ['Vista al mar panorámica', 'ROI superior al 8%', 'Ubicación premium Tenerife', 'Ático exclusivo'],
    ...
    },
    {
      ...
      roi: 7.8,
      matchScore: 88,
      reasons: ['Ático de lujo', 'Vistas al océano', 'ROI atractivo', 'Zona exclusiva'],
      image: '/placeholder.svg',
      seaView: true,
      verified: true
    },
    {
      ...
      roi: 9.1,
      matchScore: 92,
      reasons: ['ROI excepcional 9.1%', 'Vista al mar directa', 'Ubicación turística premium'],
      image: '/placeholder.svg',
      seaView: true,
      verified: true
    }
  ...
  const addAIMessage = (content: string, propertyResults?: PropertyMatch[], dossierGenerated?: boolean) => {
    const message: Message = {
      id: Date.now().toString(),
      type: 'ai',
      content,
      ...
      propertyResults,
      dossierGenerated,
      audioUrl: audioEnabled ? `/api/tts/${encodeURIComponent(content)}` : undefined
    };
    setMessages(prev => [...prev, message]);
  };
  ...
  const parseCriteria = (query: string): SearchCriteria => {
    ...
    if (priceMatch) {
      const price = parseFloat(priceMatch[1]) * (priceMatch[0].includes('M') || priceMatch[0].includes('millones') ? 1000000 : 1);
      criteria.priceRange = { min: price * 0.8, max: price * 1.2 };
    }

    // Extract location
    if (query.toLowerCase().includes('tenerife')) criteria.location = 'Tenerife';
    if (query.toLowerCase().includes('gran canaria')) criteria.location = 'Gran Canaria';
    if (query.toLowerCase().includes('lanzarote')) criteria.location = 'Lanzarote';
    ...
    if (query.toLowerCase().includes('apartamento')) criteria.propertyType = 'apartamento';

    // Extract bedrooms
    const bedroomMatch = query.match(/(\d+)\s*(?:habitaciones?|dormitorios?|bedrooms?)/i);
    ...
    // Extract features
    criteria.features = [];
    if (query.toLowerCase().includes('vista al mar') || query.toLowerCase().includes('vistas al mar')) {
      criteria.seaView = true;
      criteria.features.push('Vista al mar');
    }
    if (query.toLowerCase().includes('piscina')) criteria.features.push('Piscina');
    if (query.toLowerCase().includes('garaje')) criteria.features.push('Garaje');
  ...
  const searchProperties = (criteria: SearchCriteria): PropertyMatch[] => {
    return propertyDatabase.filter(property => {
      ...
      if (criteria.priceRange) {
        if (property.price < criteria.priceRange.min || property.price > criteria.priceRange.max) return false;
      }
      ...
      if (criteria.propertyType && !property.title.toLowerCase().includes(criteria.propertyType)) return false;
  ...
  const processAIQuery = async (query: string) => {
    ...
    if (results.length > 0) {
      const responseText = `He encontrado ${results.length} propiedades que coinciden con tus criterios. ` +
      ...
      const responseText = `No he encontrado propiedades que coincidan exactamente con tus criterios. ` +
  ...
  const generateDossier = async (properties: PropertyMatch[]) => {
    ...
    const responseText = `¡Perfecto! He generado un dossier completo con las ${properties.length} propiedades seleccionadas. ` +
  ...
  const startVoiceRecognition = () => {
    ...
    setTimeout(() => {
      ...
      setInputValue("Quiero un ático de 2 millones en Tenerife con vistas al mar y ROI mayor a 7%");
    }, 3000);
  };
      ...
      <div className="p-4 border-b border-white/10 bg-gradient-to-r from-neon-teal/20 to-neon-emerald/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/60">Búsqueda inteligente avanzada</p>
            </div>
          </div>
        ...
        {isProcessingSearch && (
          <div className="mt-3 flex items-center space-x-2 text-neon-teal">
            ...
            <span className="text-sm">Analizando criterios y buscando propiedades...</span>
          </div>
        )}
      </div>
              ...
              {message.propertyResults && (
                <div className="mt-4 space-y-3">
                  {message.propertyResults.map((property) => (
                    <div key={property.id} className="glass-card p-3 rounded-xl border border-neon-teal/20">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-bold text-white text-sm">{property.title}</h4>
                        <div className="flex items-center space-x-1">
                          <Star className="w-3 h-3 text-yellow-400" />
                          <span className="text-xs text-yellow-400">{property.matchScore}%</span>
                        </div>
                      </div>
                      
                      <p className="text-xs text-white/70 mb-2">{property.location}</p>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs mb-2">
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="text-green-400">{property.roi}% ROI</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <Bed className="w-3 h-3 text-white/60" />
                          <span className="text-white/80">{property.bedrooms} dorm</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <Square className="w-3 h-3 text-white/60" />
                          <span className="text-white/80">{property.sqm}m²</span>
                        </div>
                      </div>
              ...
              {message.dossierGenerated && (
                <div className="mt-4 glass-card p-3 rounded-xl border border-green-500/30 bg-green-500/10">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-sm font-bold text-green-400">Dossier Generado</span>
                  </div>
                  <p className="text-xs text-white/80 mb-3">
                    Documento completo con análisis financiero, comparativas y cronograma de visitas
                  </p>
