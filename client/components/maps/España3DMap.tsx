...
import { attachCityPins, flyToCity, flyThroughRegion, showRegionClusters, CITIES } from "../cesium/CityTours";
import { Eye, Globe, MapPin, Filter, Play, Pause, RotateCcw } from "lucide-react";

// Configuración de Cesium token (debe configurarse desde env)
Cesium.Ion.defaultAccessToken = import.meta.env.VITE_CESIUM_TOKEN || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJlYWE1OWUxNy1mMWZiLTQzNjItYjNkYS1kMjUyZGY0YzhlMjEiLCJpZCI6MjU5MDIsInNjb3BlcyI6WyJhc3IiLCJnYyJdLCJpYXQiOjE1NzAyNjk5Mjh9.O-8U9dT2Ns6I0aO_y5cjOYvNNxX_xWv78tPrT3Z-X8U';
...
export default function España3DMap({ 
  ...
  const startCinematicTour = (region: 'peninsula' | 'canarias' | 'baleares') => {
  ...
  const getRegionStats = (region: string) => {
    const regionCities = CITIES.filter(c => c.region === region);
    const totalProperties = regionCities.reduce((sum, c) => sum + (c.properties || 0), 0);
      ...
      <div className="absolute top-4 left-4 right-4 z-10 flex justify-between items-start">
        ...
        {selectedCity && (
          <div className="glass-card px-4 py-3 rounded-xl max-w-xs">
            ...
            <p className="text-white/70 text-sm capitalize">{selectedCity.region}</p>
            <p className="text-white text-sm">{selectedCity.properties} propiedades</p>
          </div>
        )}
      </div>
            ...
            {['peninsula', 'canarias', 'baleares'].map((region) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs">{stats.cities} ciudades • {stats.properties} props</div>
                </button>
              );
            })}
          </div>
      ...
      <div className="absolute bottom-4 left-4 right-4 z-10">
        <div className="glass-card p-4 rounded-xl">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-neon-teal">{CITIES.length}</div>
