...
export default function DigitalTwin({ property, vrTourUrl }: DigitalTwinProps) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setEnergyData((prev) => ({
        ...prev,
        currentConsumption: Math.max(
          0.5,
          prev.currentConsumption + (Math.random() - 0.5) * 0.2,
        ),
        solarGeneration: Math.max(
          0,
          (prev.solarGeneration || 0) + (Math.random() - 0.5) * 20,
        ),
      }));
    }, 3000);
          ...
          <div className="glass p-3 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <span className="text-white font-bold">
                {energyData.solarGeneration?.toFixed(1)}W
              </span>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="glass p-4 rounded-xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-2">
                  <div
                    className={`text-2xl font-bold ${getEfficiencyColor(energyData.efficiency)}`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="glass p-4 rounded-xl">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-white">
                      {energyData.solarGeneration?.toFixed(0)}
                    </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm text-white/60">
                Total invertido:{" "}
                {formatCurrency(
                  reformHistory.reduce((sum, reform) => sum + reform.cost, 0),
                )}
              </div>
            </div>
                        ...
                        {reform.energyImpact && (
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-neon-teal text-sm">
                              +{reform.energyImpact}% eficiencia
                            </span>
                          </div>
                        )}
                      </div>
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  <div className="text-2xl font-bold text-neon-teal">
                    {formatCurrency(communityData.monthlyCost)}
                  </div>
                  <div className="text-sm text-white/60">Mensual</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-white">
                    {formatCurrency(communityData.yearlyBudget)}
                  </div>
                  <div className="text-sm text-white/60">Presupuesto anual</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-400">
                    {formatCurrency(communityData.reserves)}
                  </div>
            ...
            <div className="glass p-4 rounded-xl">
              <h5 className="font-bold text-white mb-3">
                Salud Financiera de la Comunidad
              </h5>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-white/70">
                    Ratio mantenimiento/ingresos:
                  </span>
                  <span className="text-white font-bold">
                    {(communityData.maintenanceRatio * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-white/70">
                    Reservas vs. presupuesto anual:
                  </span>
                  <span className="text-green-400 font-bold">
                    {(
                      (communityData.reserves / communityData.yearlyBudget) *
                      100
                    ).toFixed(0)}
                    %
                  </span>
          ...
          <div className="w-full h-full max-w-6xl max-h-4xl bg-blue-dark rounded-lg overflow-hidden">
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              <h3 className="text-white font-bold">
                Tour VR con Datos en Tiempo Real
              </h3>
              <button
