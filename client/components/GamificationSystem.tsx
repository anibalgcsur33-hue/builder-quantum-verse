...
export default function GamificationSystem({ userStats, compact = false }: GamificationSystemProps) {
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-3">
            <div className={`w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 p-2 flex items-center justify-center`}>
              <currentLevel.icon className={`w-full h-full ${currentLevel.color}`} />
            </div>
            <div>
              <div className="font-bold text-white">{currentLevel.name}</div>
              <div className="text-sm text-white/60">{userStats.points} puntos</div>
            </div>
          </div>
          <div className="text-right">
            <div className="flex items-center space-x-1">
              <Coins className="w-4 h-4 text-yellow-400" />
              <span className="font-bold text-yellow-400">{userStats.behTokens}</span>
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 rounded-2xl border border-purple-500/20">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-2xl font-bold text-white">{currentLevel.name}</h3>
              <p className="text-white/70">{currentLevel.description}</p>
              <div className="flex items-center space-x-4 mt-2">
                <div className="flex items-center space-x-1">
                  <Star className="w-4 h-4 text-yellow-400" />
                  <span className="text-white font-medium">{userStats.points} puntos</span>
                </div>
                <div className="flex items-center space-x-1">
                  <Trophy className="w-4 h-4 text-purple-400" />
                  <span className="text-white/60">Rango #{userStats.rank} de {userStats.totalUsers}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="text-right">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-2xl font-bold text-yellow-400">{userStats.behTokens}</span>
            </div>
        ...
        {nextLevel && (
          <div>
            <div className="flex justify-between mb-2">
              ...
              <span className="text-white/70">{userStats.points} / {nextLevel.minPoints}</span>
            </div>
            ...
            <div className="text-sm text-white/60">
              {nextLevel.minPoints - userStats.points} puntos para el siguiente nivel
            </div>
          </div>
        )}
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 rounded-2xl">
            <h4 className="text-xl font-bold text-white mb-4">Cómo ganar puntos</h4>
              ...
              {pointsActivities.map((activity, index) => {
                ...
                return (
                  <div key={index} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                    ...
                    <div className="flex items-center space-x-1">
                      <span className="font-bold text-purple-400">+{activity.points}</span>
                      <Star className="w-4 h-4 text-yellow-400" />
                    </div>
                  </div>
                );
          ...
          <div className="glass-card p-6 rounded-2xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-white/5 rounded-lg">
                ...
                <div className="text-2xl font-bold text-white">{userStats.activities.posts}</div>
                <div className="text-white/60 text-sm">Posts</div>
              </div>
              <div className="text-center p-4 bg-white/5 rounded-lg">
                <MessageCircle className="w-8 h-8 text-green-400 mx-auto mb-2" />
                <div className="text-2xl font-bold text-white">{userStats.activities.comments}</div>
                <div className="text-white/60 text-sm">Comentarios</div>
              </div>
              <div className="text-center p-4 bg-white/5 rounded-lg">
                <CheckCircle className="w-8 h-8 text-purple-400 mx-auto mb-2" />
                <div className="text-2xl font-bold text-white">{userStats.activities.verifications}</div>
                <div className="text-white/60 text-sm">Verificaciones</div>
              </div>
              <div className="text-center p-4 bg-white/5 rounded-lg">
                <UserPlus className="w-8 h-8 text-yellow-400 mx-auto mb-2" />
                <div className="text-2xl font-bold text-white">{userStats.activities.invites}</div>
                <div className="text-white/60 text-sm">Invitaciones</div>
              </div>
              <div className="text-center p-4 bg-white/5 rounded-lg">
                <Home className="w-8 h-8 text-red-400 mx-auto mb-2" />
                <div className="text-2xl font-bold text-white">{userStats.activities.operationsClosed}</div>
                <div className="text-white/60 text-sm">Operaciones</div>
              </div>
              <div className="text-center p-4 bg-white/5 rounded-lg">
                <Eye className="w-8 h-8 text-cyan-400 mx-auto mb-2" />
                <div className="text-2xl font-bold text-white">{userStats.activities.vrToursCompleted}</div>
      ...
      {activeTab === 'achievements' && (
        <div className="glass-card p-6 rounded-2xl">
          <h4 className="text-xl font-bold text-white mb-6">Logros desbloqueados</h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center space-x-1">
                      <Star className="w-4 h-4 text-yellow-400" />
                      <span className="text-yellow-400 font-medium">{achievement.points}</span>
                    </div>
      ...
      {activeTab === 'rewards' && (
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 rounded-2xl">
            <h4 className="text-xl font-bold text-white mb-4">Beneficios de tu nivel actual</h4>
            ...
            <div className="glass-card p-6 rounded-2xl border border-purple-500/20">
              ...
              <div className="mt-4 p-3 bg-purple-500/20 rounded-lg">
                <div className="text-purple-400 font-medium">
                  ¡Faltan solo {nextLevel.minPoints - userStats.points} puntos para desbloquearlo!
                </div>
              </div>
            </div>
          )}
          ...
          {/* Token Marketplace */}
          <div className="glass-card p-6 rounded-2xl">
            <h4 className="text-xl font-bold text-white mb-4">Canjear BEH Tokens</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-white/5 rounded-lg">
                ...
                <button className="w-full btn-secondary text-sm py-2">Canjear</button>
              </div>
              <div className="p-4 bg-white/5 rounded-lg">
                <div className="flex justify-between items-center mb-2">
