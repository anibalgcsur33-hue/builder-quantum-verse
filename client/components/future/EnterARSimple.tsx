...
export default function EnterARSimple() {
  ...
  const startAR = async () => {
    ...
    try {
      ...
      alert('AR Mode: Apunta tu dispositivo hacia una superficie plana y toca para colocar la propiedad virtual');
      
      // Simulate AR session
      setTimeout(() => {
        setArActive(false);
        alert('Demo AR completada. En la versión completa verías el modelo 3D de la propiedad.');
      }, 5000);
    } catch (error) {
  ...
  return (
    <>
      <button 
        ...
        disabled={arActive}
      >
        {arActive ? 'AR Activo...' : supported ? 'Entrar en AR' : 'Ver en 3D/VR'}
      </button>
