      ...
      const loop = () => {
        ...
        barsRef.current.forEach((bar, i) => {
          if (bar) {
            ...
            if (level > 0.7) {
              bar.style.background = 'linear-gradient(to top, #ef4444, #f87171)'; // red
            } else if (level > 0.4) {
              bar.style.background = 'linear-gradient(to top, #f59e0b, #fbbf24)'; // orange
            } else {
              bar.style.background = 'linear-gradient(to top, #06b6d4, #67e8f9)'; // cyan
            }
          }
        });
  ...
  function stop() {
    ...
    barsRef.current.forEach(bar => {
      if (bar) {
        ...
        bar.style.background = 'linear-gradient(to top, #06b6d4/20, #67e8f9/70)';
      }
    });
  }
