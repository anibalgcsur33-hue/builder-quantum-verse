...
const CAN_STT = "webkitSpeechRecognition" in (window as any) || "SpeechRecognition" in (window as any);

export default function AIConcierge() {
  const [open, setOpen] = useState(false);
  const [list, setList] = useState<Msg[]>([
    { role: "assistant", text: "Hola, soy BlueEye. ¿Qué propiedad te muestro en VR o AR?" },
  ]);
  ...
  function speak(text: string) {
    ...
    const v = speechSynthesis.getVoices().find(v => /es|spanish/i.test(v.lang));
    if (v) utter.voice = v;
    utter.rate = 0.98; utter.pitch = 1.05; utter.volume = 0.9;
    utter.onstart = () => {
  ...
  function fakeAI(answerTo: string) {
    ...
    if (q.includes("tener") || q.includes("tenerife")) return "Tengo 14 villas en Tenerife con VR listos. ¿Te muestro las vistas al mar primero o las de acantilado?";
    if (q.includes("marbella")) return "Marbella tiene 8 propiedades premium disponibles. ¿Prefieres villa con piscina infinita o penthouse con terraza?";
    if (q.includes("madrid")) return "En Madrid tengo 12 penthouses exclusivos en Salamanca y Chamberí. ¿Quieres ver el tour virtual 360°?";
    if (q.includes("barcelona")) return "Barcelona ofrece 6 lofts de lujo en Eixample. ¿Te interesa ver las vistas al Sagrada Familia?";
    if (q.includes("ar") || q.includes("realidad aumentada")) return "Pulsa 'Entrar en AR' y coloca la maqueta en tu mesa. Te acompaño paso a paso con instrucciones de voz.";
    if (q.includes("vr") || q.includes("virtual")) return "Activando portal VR inmersivo. ¿Tienes cascos VR o prefieres modo navegador 3D?";
    if (q.includes("dossier") || q.includes("información")) return "Puedo crear un dossier PDF premium con fotos 4K, mapa 3D interactivo y proyección de ROI a 5 y 10 años. ¿Para qué propiedad?";
    if (q.includes("hipoteca") || q.includes("financi") || q.includes("préstamo")) return "Puedo iniciar preaprobación bancaria express en 2 minutos. ¿País de residencia fiscal y rango presupuestario?";
    if (q.includes("metaverso")) return "Abriendo portal metaverso con tour guiado premium. Prepárate para una experiencia única.";
    if (q.includes("precio") || q.includes("cost")) return "Los precios van desde €850K hasta €12M. ¿Qué rango de inversión tienes en mente?";
    if (q.includes("canarias")) return "Las Islas Canarias tienen beneficios fiscales únicos. ¿Te interesa la Zona Especial Canaria para inversores?";
    if (q.includes("invertir") || q.includes("inversión")) return "Como experto en inversión inmobiliaria premium, puedo mostrarte propiedades con ROI del 8-12% anual. ¿Prefieres rentabilidad o plusvalía?";
          ...
          <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between bg-gradient-to-r from-cyan-500/10 to-purple-500/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-white/60">Asistente Premium IA</div>
              </div>
            </div>
